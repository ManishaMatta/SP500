...
class CommonModule:
    ...
    def date_generator(date_format, interval):
        ed = datetime.now().strftime(date_format)
        sd = (datetime.now() - timedelta(days=interval)).strftime(date_format)
        return tuple([sd, ed])
    ...
    def ds_api(web_link, json_path='', mode='d'):
        ...
        if req.status_code == 200:
            ...
            return df if mode == 'd' else df.head(100)
    ...
    def company_yahoo_hist(cmpy_names):
        ...
        for cmpy in cmpy_names:
            cmp_df = yf.download(tickers=cmpy, period='100d', interval='1d')[['Close', 'Volume']].add_prefix(cmpy+"_")
    ...
    def stock_details(link, ds_link, cmpy_names):
        ...
        if sp_res.status_code == 200:
            ...
            print("error in response from page")
    ...
    def company_details(link, href):
        ...
        try:
            ...
            if req.status_code == 200:
                ...
                cmpy_dtls += (article,)
                # forecast
                for fcast in parser.findAll("details", {
                    "class": "fontsize-12 border-white margin-top--smaller padding-left--smaller"}):
